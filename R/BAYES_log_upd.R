...
fit.AR.log <- function(data, order = 1, Total_itr = 5000, burn = 2500){
  ...
  At <- 0
  fit.G <- tsglm(Y, model=list(past_obs=1), init.method='firstobs',
                 final.control=list(NULL), link='log', distr = "poisson")
...
fit.INGARCH.log <- function(data, order1 = 1, order2 = 1, Total_itr = 5000, burn = 2500){
  ...
  grad_UK <- function(x, deltaRr = deltaR, tempsig=sigma2lat){
    ...
    #expxdev <- diag(exp(x)/sum(exp(x))) - tcrossprod(exp(x), exp(x)) /  (sum(exp(x)))^2
    #return(array(expxdev %*% compo) + (x) / (100))
    return(2*sum(array(compo)*xcder)+ xc/100)
  }
  ...
  fit.G <- tsglm(Y, model=list(past_obs=1, past_mean=1), init.method='firstobs',
                 final.control=list(NULL), distr = "poisson", link = "log")
  deltamu <- fit.G$coefficients[1]
  deltaA <- fit.G$coefficients[2]
  deltaB <- fit.G$coefficients[3]
  
  deltaR <- (deltaA+deltaB)
  deltaK <- (deltaA-deltaB)
  
  deltaRu <- log((1-deltaR)/(1+deltaR))
  deltaKu <- log((1-deltaK/2)/(1+deltaK/2))
  ...
  while(itr < Total_itr){
    ...
    deltaKu <- temp$up
    arKu <- temp$arc
    deltaK <- 2*(1-exp(deltaKu))/(1+exp(deltaKu))
...
fit.INGARCHX.log <- function(data, Xreg, order1 = 1, order2 = 1, Total_itr = 5000, burn = 2500){
  ...
  grad_UK <- function(x, deltaRr = deltaR, deltaCc=deltaC, tempsig=sigma2lat){
    ...
    xcder <- - exp(xc)*(1-exp(xc))/(1+exp(xc))^2 - exp(xc)/(1+exp(xc)) 
    return(sum(2*array(compo)*xcder)+ xc/100)
  }
 ...
 # Data and priors
  ...
  sigma2lat <- log(fit.G$fitted.values[1])#rnorm(1)#
